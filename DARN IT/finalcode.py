...
class ImageUploader(QWidget):
    def __init__(self):
        ...
        self.setWindowTitle('DARN IT')
        self.setGeometry(650, 250, 500, 600)
        ...
        self.select_area_button.setStyleSheet(style_sheet)
        ...
        self.text_input_button.setStyleSheet(style_sheet)
        self.process_text_button = QPushButton('Process Text')
        self.process_text_button.setFont(QFont('Arial', 14))
        self.process_text_button.setStyleSheet(style_sheet)
    ...
    def input_text(self):
        text_input = QInputDialog(self)
        text_input.setFixedSize(300, 100)
        text, ok = text_input.getText(self, 'Input Text', 'Enter text:')
        ...
        while True:
            ...
            if w and h:
                ...
                pixmap = QPixmap.fromImage(
                    QImage(self.selected_img.tobytes(), self.selected_img.shape[1], self.selected_img.shape[0],
    ...
    def process_image(self):
        ...
        text_clf = load('C:\\Users\\darsh\\Downloads\\DarnIT-main (1)\\DarnIT-main\\DARN IT\\text_classifier.joblib')
        # Use the model to predict the category of the input text
        predicted_category = text_clf.predict([text])[0]
        if predicted_category == 1:
            self.category_label.setText(f"Predicted Module {predicted_category} : Introduction")
        if predicted_category == 2:
            self.category_label.setText(f"Predicted Module {predicted_category} : Physical Layer and Data Link Layer ")
        if predicted_category == 3:
            self.category_label.setText(f"Predicted Module {predicted_category} : Network Layer")
        if predicted_category == 4:
            self.category_label.setText(f"Predicted Module {predicted_category} : Transport Layer and session Layer ")
        if predicted_category == 5:
            self.category_label.setText(
                f"Predicted Module {predicted_category} : Presentation Layer and Application Layer")
        if predicted_category == 6:
            self.category_label.setText(f"Predicted Module {predicted_category} : Network Design Concepts")
        self.text_label.setText(f"Entered Text: {text}")
